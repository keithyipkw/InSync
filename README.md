...
It is better than documenting the methods and hope that callers do right.
...
If a simple reference to a `Synchronized<T>` is written by a thread then read by another thread, it is still not thread-safe:
...
The variable should have some synchronizations. For the above exmaple, `volatile` is sufficient:
...
However, `volatile` is usually wrong for more complicated usages. `Synchronized<ValueContainer<T>>` is a better choice:
